    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Digital System address (e.g. DACLambeGgkxodKtfzM7huFXyPYWjz33Z2)"));
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CDigitalsystemAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 500 DSYS.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    ...
    case WalletModel::InsaneFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an digitalsystemly high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CDigitalsystemAddress addr = CDigitalsystemAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Digitalsystem address"));
        }
        else // Valid address
        {
            CPubKey pubkey;

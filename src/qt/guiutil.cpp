...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Digitalsystem"))
        return false;
...
{
    // Convert Digitalsystem:// to Digitalsystem:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Digitalsystem://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Digitalsystem:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Digitalsystem.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Digitalsystem.lnk
...
{
    return GetAutostartDir() / "Digitalsystem.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Digitalsystem.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Digital System\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Digitalsystem app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Digitalsystem app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Digitalsystem-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Digitalsystem-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Digitalsystem-Qt"));

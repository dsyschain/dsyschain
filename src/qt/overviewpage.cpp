...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDigitalsystemAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDigitalsystemAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDigitalsystemAmount = strAnonymizeDigitalsystemAmount.remove(strAnonymizeDigitalsystemAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalsystemAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDigitalsystemAmount*COIN) nMaxToAnonymize = nAnonymizeDigitalsystemAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDigitalsystemAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDigitalsystemAmount));
        strAnonymizeDigitalsystemAmount = strAnonymizeDigitalsystemAmount.remove(strAnonymizeDigitalsystemAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalsystemAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDigitalsystemAmount == 0){

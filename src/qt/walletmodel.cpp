...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CDigitalsystemAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CDigitalsystemAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject digitalsystem fee
        unsigned int digitalsystemfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        digitalsystemfee = ((MIN_RELAY_TX_FEE * digitalsystemfee) * 10000);
        if (nFeeRequired > digitalsystemfee){
            LogPrintf("nFeeRequired: %d -- InsaneFee: %d\n", nFeeRequired, digitalsystemfee);
            return InsaneFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CDigitalsystemAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CDigitalsystemAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CDigitalsystemAddress(address).ToString());
...
{
    // Digitalsystem context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CDigitalsystemAddress(address).ToString())].push_back(out);
    }
}

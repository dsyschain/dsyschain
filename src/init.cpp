...
void Shutdown()
{
    ...
    RenameThread("Digitalsystem-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Digitalsystem.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Digitalsystemd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDigitalsystemamount=<n> " + _("Keep N Digitalsystem anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Digitalsystem is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Digitalsystem is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Digitalsystem version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Digitalsystem server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Digitalsystem") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Digitalsystem to complete") << "\n";
    ...
    nAnonymizeDigitalsystemAmount = GetArg("-anonymizeDigitalsystemamount", 0);
    if(nAnonymizeDigitalsystemAmount > 999999) nAnonymizeDigitalsystemAmount = 999999;
    if(nAnonymizeDigitalsystemAmount < 2) nAnonymizeDigitalsystemAmount = 2;
    ...
    LogPrintf("Anonymize Digitalsystem Amount %d\n", nAnonymizeDigitalsystemAmount);

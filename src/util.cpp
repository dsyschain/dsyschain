...
int nAnonymizeDigitalsystemAmount = 1000;
...
#else
    const char* pszModule = "Digitalsystem";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DSYS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DSYS
    // Mac: ~/Library/Application Support/DSYS
    // Unix: ~/.DSYS
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DSYS";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Digitalsystem.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Digitalsystem.conf";
               FILE* ConfFile = fopen(ConfPath.string().c_str(), "w");

               fprintf(ConfFile, "server=1\n");

               fprintf(ConfFile, "rpcuser=digitalsystem\n");
               ...
               fprintf(ConfFile, "addnode=dsys1.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys2.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys3.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys4.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys5.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys6.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys7.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys8.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys9.dsyschain.com\n");
		fprintf(ConfFile, "addnode=dsys10.dsyschain.com\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Digitalsystemd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Digitalsystem will not work properly.");
